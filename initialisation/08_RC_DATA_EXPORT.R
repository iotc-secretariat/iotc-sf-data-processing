...
## Wide table format
FIELDS_STD_RC_DATA_TABLE = data.table(read.xlsx("../inputs/FIELDS_STANDARDIZED_RC_DATA.xlsx", sheet = "RC FORMAT FIELDS", sep.names = " "))[, .(FIELD, DEFINITION, `CODE LIST NAME`, `CODE LIST URL`)]

FIELDS_STD_RC_DATA_TABLE[!is.na(`CODE LIST NAME`), `CODE LIST` := paste0("<a href=\"", `CODE LIST URL`, "\">", `CODE LIST NAME`, "</a>")]

FIELDS_STD_RC_DATA_TABLE = FIELDS_STD_RC_DATA_TABLE[, -c("CODE LIST NAME", "CODE LIST URL")]
...
writeDataTable(STD_SF_DATA, sheet = "FLEETS", x = CL_FLEETS,  tableStyle = "TableStyleLight11")
setColWidths(STD_SF_DATA, sheet = "FLEETS", cols = 1:nrow(CL_FLEETS), widths = "auto")

## IRREGULAR GRID ####
addWorksheet(STD_SF_DATA, "FISHING GROUNDS")
writeDataTable(STD_SF_DATA, sheet = "FISHING GROUNDS", x = CL_FISHING_GROUNDS,  tableStyle = "TableStyleLight11")
setColWidths(STD_SF_DATA, sheet = "FISHING GROUNDS", cols = 1:nrow(CL_FISHING_GROUNDS), widths = "auto")

## GEARS ####
addWorksheet(STD_SF_DATA, "GEARS")
writeDataTable(STD_SF_DATA, sheet = "GEARS", x = CL_GEARS,  tableStyle = "TableStyleLight11")
setColWidths(STD_SF_DATA, sheet = "GEARS", cols = 1:nrow(CL_GEARS), widths = "auto")

## SCHOOL TYPES ####
addWorksheet(STD_SF_DATA, "SCHOOL TYPES")
writeDataTable(STD_SF_DATA, "SCHOOL TYPES", CL_SCHOOL_TYPES, tableStyle = "TableStyleLight11")
setColWidths(STD_SF_DATA, sheet = "SCHOOL TYPES", cols = 1:nrow(CL_SCHOOL_TYPES), widths = "auto")

## MEASUREMENT TYPES ####
addWorksheet(STD_SF_DATA, "MEASUREMENTS")
writeDataTable(STD_SF_DATA, "MEASUREMENTS", CL_MEASUREMENTS, tableStyle = "TableStyleLight11")
setColWidths(STD_SF_DATA, sheet = "MEASUREMENTS", cols = 1:nrow(CL_MEASUREMENTS), widths = "auto")

## RAISINGS ####
addWorksheet(STD_SF_DATA, "RAISINGS")
writeDataTable(STD_SF_DATA, "RAISINGS", CL_RAISINGS, tableStyle = "TableStyleLight11")
setColWidths(STD_SF_DATA, sheet = "RAISINGS", cols = 1:nrow(CL_RAISINGS), widths = "auto")

# Save the workbook
saveWorkbook(STD_SF_DATA, paste0("../outputs/", TITLE, "-DDD.xlsx"), overwrite = TRUE)

# EXPORT DATA AS CSV FILE ####

## MELTED FORMAT ####
write.csv(FL_STD_DATA_SPECIES, file = paste0("../outputs/", TITLE, "-MELTED-FORMAT.csv"), row.names = FALSE)

# File size in KB
STD_SF_MELTED_DATA_SIZE = round(file.size(paste0("../outputs/", TITLE, "-MELTED-FORMAT.csv"))/1e3)

## WIDE FORMAT ####
write.csv(FL_STD_DATA_SPECIES_TABLE, file = paste0("../outputs/", TITLE, "-WIDE-FORMAT.csv"), row.names = FALSE)

# File size in KB
STD_SF_TABLE_DATA_SIZE = round(file.size(paste0("../outputs/", TITLE, "-WIDE-FORMAT.csv"))/1e3)

# Zip the files
zip::zip(zipfile = paste0("../outputs/", TITLE, ".zip"), files = c(paste0("../outputs/", TITLE, "-WIDE-FORMAT.csv"), paste0("../outputs/", TITLE, "-MELTED-FORMAT.csv"), paste0("../outputs/", TITLE, "-DDD.xlsx")), mode = "cherry-pick", recurse = FALSE)
         
# Zipped file size in KB
STD_SF_DATA_ZIP_SIZE = round(file.size(paste0("../outputs/", TITLE, ".zip"))/1e3)

# Remove files after creating the archive
file.remove(paste0("../outputs/", TITLE, "-DDD.xlsx"))
file.remove(paste0("../outputs/", TITLE, "-MELTED-FORMAT.csv"))
file.remove(paste0("../outputs/", TITLE, "-WIDE-FORMAT.csv"))
