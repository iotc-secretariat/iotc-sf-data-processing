...
if(!file.exists("../inputs/shapes/IO_SA_YFT_1.0.0_SHP.zip")){
  download.file("https://data.iotc.org/reference/latest/domain/admin/shapefiles/IO_SA_YFT_1.0.0_SHP.zip", destfile = "../inputs/shapes/IO_SA_YFT_1.0.0_SHP.zip", mode = "wb")
  unzip("../inputs/shapes/IO_SA_YFT_1.0.0_SHP.zip", exdir = "../inputs/shapes/")}

YFT_SA_AREAS_SF = st_make_valid(st_read("../inputs/shapes/IO_SA_YFT_1.0.0.shp"))

# Centroids for labels
YFT_SA_AREAS_CENTROIDS_SF = st_centroid(YFT_SA_AREAS_SF)
YFT_SA_AREAS_CENTROIDS_SF = YFT_SA_AREAS_CENTROIDS_SF %>% mutate(CODE = gsub("SA_YFT_", "", CODE))
...
# LIST_SF_NON_STANDARD_AREAS derived in 02_DATA_PROCESS.R
SF_NON_STANDARD_AREAS =
  query(
    ...
    FROM
      refs_gis.AREAS
    WHERE
      CODE IN ('", paste(LIST_NON_STANDARD_AREAS$FISHING_GROUND_CODE, collapse = "', '"), "')"
    ))
